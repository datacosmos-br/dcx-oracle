...
Optimization:
  --optimize            Enable size-based optimization (Ants vs Elephants)
...
mig_load_config() {
	...
	# 1. Try to load secure credentials from dcx cred (Phase 4 integration)
	...
	# 2. Load configuration hierarchically: defaults -> global -> local -> env
	...
	# If using Wallet, password might be empty/unnecessary
	if [[ -n "${ORACLE_WALLET_LOCATION:-}" ]] || [[ "${DB_CONNECTION_STRING:-}" == /@* ]]; then
		runtime_require_vars "DB_CONNECTION_STRING"
		# If password is provided, use it; otherwise assume wallet auth
		if [[ -n "${DB_ADMIN_PASSWORD:-}" ]]; then
			CONNECTION="${DB_ADMIN_USER}/${DB_ADMIN_PASSWORD}@${DB_CONNECTION_STRING}"
		else
			CONNECTION="/@${DB_CONNECTION_STRING}"
			log_info "Using Wallet authentication (no password)"
		fi
	else
...
mig_main() {
	...
	if [[ "$USE_DUMPFILES" -eq 1 ]]; then
		...
		dp_execute_batch_optimized \
			...
			--scn "$scn" \
			--directory "$DIRECTORY" \
			--max-concurrent "${MAX_CONCURRENT_PROCESSES:-4}" \
			${METADATA_ONLY:+"--metadata-only"} || exit_code=$?
	else
